...
- **Filenames/version** come from `targetModel.acronym` + `majorVersion`/`minorVersion`.
  All CDIF profiles + composites are currently `1`/`1` (matching the `/1.1/`
  conformance URIs in mBB; bumped 2026-06 from `/1.0/`). `build-docs.ps1`
  derives the canonical filename (`<acronym>_<maj>-<min>_canonical-unique-names.xmi`).
